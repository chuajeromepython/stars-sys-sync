...
class ECDCController extends Controller
{
    public function index(){
        ...
        foreach($classrooms["Kinder"] as $key => $classroom){
            ...
            $classrooms["Kinder"][$key]["details"] = array(
                "bosy" => $bosy,
                "eosy" => $eosy,
                "mosy" => $mosy,
            );
    ...
    public function store(Request $request){
        ...
        try {
            ...
            foreach ($competencies as $key => $competency) {
                ...
                $p = isset($request->p[$competency->id]) ? 1 : 0;
                $o = isset($request->o[$competency->id]) ? 1 : 0;
                $r = isset($request->r[$competency->id]) ? 1 : 0;
                $score = ($p == 1 || $o == 1 || $r == 1 ) ? 1 : 0;
    ...
    public function card(Classroom $classroom, Student $student){
        ...
        $domains = ECDCDomain::all();
        $domain_competencies = array();

        foreach ($domains as $key => $domain) {
            $domain_competencies[$domain->id] = array(
                "domain" => $domain->domain,
                "competencies" => ECDCCompetency::where("domain_id", $domain->id)->get()
            );
        }
    ...
    public function download_template($classroom_id){
        ...
        foreach ($students as $key => $student) {
            ...
            $sheet->setCellValue($col++.'3', "P");
            $sheet->setCellValue($col++.'3', "O");
            $sheet->setCellValue($col++.'3', "R");
                ...
                if($check_lrn->count() > 0){
                    for ($row=5; $row < 124 ; $row++) {
                        ...
                        if($competency_id != '*'){
                            ...
                            $p = $spreadsheet->getActiveSheet()->getCell($p_col.$row)->getValue();
                            $o = $spreadsheet->getActiveSheet()->getCell($o_col.$row)->getValue();
                            $r = $spreadsheet->getActiveSheet()->getCell($r_col.$row)->getValue();

                            $data[$check_lrn[0]->student_id][$competency_id] = array(
                                'p' => $p,
                                'o' => $o,
                                'r' => $r
                            );
                ...
                foreach ($data as $student_id => $competencies) {
                    ...
                    if($existing > 0){
                        ...
                        foreach ($competencies as $competency_id => $points) {
                            ...
                            $score = ($points['p'] == 1 || $points['o'] == 1 || $points['r'] == 1 ) ? 1 : 0;
                            $student_ecdc = new StudentECDC;
                            $student_ecdc->ecdc_id = $ecdc->id; 
                            $student_ecdc->student_id = $student_id;
                            $student_ecdc->ecdc_competency_id = $competency_id;
                            $student_ecdc->p = $points['p'];
                            $student_ecdc->o = $points['o'];
                            $student_ecdc->r = $points['r'];
                            $student_ecdc->score = $score;
                            $student_ecdc->save();
    ...
    public function print(){
        return view('ecdc.reports.student_result');
    }
    ...
    public function print_front($ecdc_id, $student_id){
        ...
        return view('ecdc.reports.front',
            compact('data', 'result')
        );
    }
